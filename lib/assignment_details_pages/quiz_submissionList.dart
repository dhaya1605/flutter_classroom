...
import 'package:eclass/assignment_details_pages/quiz_evaluation_page.dart';
import 'package:flutter/material.dart';
import 'dart:ui' as ui;
import 'package:cloud_firestore/cloud_firestore.dart';


class quiz_submissionList extends StatefulWidget {
  quiz_submissionList(this.classroomId,this.title);
  String classroomId;
  String title;
  @override
  _quiz_submissionListState createState() => _quiz_submissionListState(classroomId,title);
}

class _quiz_submissionListState extends State<quiz_submissionList> {
  _quiz_submissionListState(this.classroomId,this.title);
  String classroomId;
  String title;
  String n ='';

  numberOfSubmission()async{
    QuerySnapshot numbers = await Firestore.instance.collection('classrooms').document(classroomId).collection('quiz').document(title).collection('submissions').getDocuments();
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: <Widget>[
            ...
            StreamBuilder(
                stream: Firestore.instance.collection('classrooms').document(classroomId).collection('quiz').document(title).collection('submissions').snapshots(),
...
//                    n = len<10? "0${len.toString()}" : "${len.toString()}";
                    ...
                    for(var Class in classes){
                      ...
                      final imageWidget = Student_detail(time,email,date,classroomId,title,evaluated);
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        GestureDetector(
          onTap: (){
            Navigator.push(context, MaterialPageRoute(builder: (context)=>quiz_evaluation(classroomId,title,email)));
          },
          child: Container(
